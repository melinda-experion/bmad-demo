...
3. Confidence score: self-assess per `references/confidence-scoring.md` and write `confidence`, `confidence_label`, `confidence_rationale` into `brief.md` frontmatter. Surface the score and rationale to the user alongside the readiness message in the next step.
...
5. Tell the user it is ready: local paths and external destinations (URLs returned from handoffs), and the confidence score with its rationale. Invoke `bmad-help` to suggest what next steps make sense in the bmad method ecosystem.
