...
- **Confidence score.** Self-assess per `references/confidence-scoring.md` and write `confidence`, `confidence_label`, `confidence_rationale` into `EXPERIENCE.md` frontmatter only — `DESIGN.md`'s frontmatter is locked to the external design.md spec and does not gain extra keys.
