...
Before writing, compute `target_version`: `prd.md`'s current frontmatter `version` (0 if absent) plus one — the version the PRD will carry once next approved, and so the version this review is actually reviewing.

Write findings to `{doc_workspace}/review-rubric.md`, with frontmatter `prd_version: {target_version}` (plus `title` and `date`) ahead of the report body — this replaces whatever was there for a prior version:

```markdown
---
title: PRD Quality Review — {prd_name}
prd_version: {target_version}
date: {date}
---
