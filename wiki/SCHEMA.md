...
This wiki documents project knowledge for the `bmad-demo` repo, not the code itself (code is derivable from git). It uses these page types, each with a dedicated subdirectory:

- `architecture` (in `wiki/architecture/`) — architecture decisions, system design notes, approved architecture snapshots.
- `bug` (in `wiki/bugs/`) — bug reports and postmortems.
- `concept` (in `wiki/concepts/`) — ideas, methods, frameworks, abstractions (e.g. BMAD workflow terms).
- `decision` (in `wiki/decisions/`) — standalone decisions and their rationale, outside architecture.
- `glossary` (in `wiki/glossary/`) — short definitions of project-specific terms.
- `lesson` (in `wiki/lessons/`) — lessons learned, retrospective takeaways.
- `person` (in `wiki/people/`) — pages about people involved in the project.
- `reference` (in `wiki/references/`) — pointers to external systems (trackers, dashboards, docs).
- `workflow` (in `wiki/workflows/`) — process/workflow notes (e.g. BMAD skill sequencing).
- `source` (in `wiki/sources/`) — one summary page per ingested raw source.
- `entity` (in `wiki/entities/`) — pages about specific things not covered above: products, papers, places, organizations.
