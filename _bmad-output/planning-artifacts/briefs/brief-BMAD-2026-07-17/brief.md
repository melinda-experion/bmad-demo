...
Build a micro web app called BMAD Idea Launcher: the user enters a short problem or experiment prompt, clicks `Generate ideas`, and gets three idea cards with a favorite toggle — all on one responsive screen, all state session-only. See Scope below for the full feature list. This makes the ideation step feel concrete, fast, and repeatable.
