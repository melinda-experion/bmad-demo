...
2. **Approval gate** — you cannot generate a PRD until the brief is approved *and* unchanged since approval (hash check).
3. **Plan-first hard gate** — no code is generated until you type an exact approval sentence for the implementation plan.
4. **Confidence scoring** — every document gets an LLM self-assessed score; every generated *file* gets a deterministic score computed from tests, lint, diff size, and revision count.
...
| Ollama *(optional)* | PromptGateway's LLM risk classifier | `ollama list` |
...
> The current repo contains a *completed* run (the "BMAD Idea Launcher" app plus its artifacts).
...
`persistent_facts` that reference the *old* app's context files (`.ai-context-idea-generation.md`,
...
- **Persistent fact:** any substantive PRD edit *later in chat* auto-reverts `approved` → `draft` and logs the change immediately.
...
- **Persistent fact:** the Reviewer Gate's consolidated report is always written to the fixed path
  ...
  files under `reviews/` (adversarial, tech-currency, rubric) are *not* canonical.
...
architecture decision, e.g. *"Do NOT introduce a web framework — Node core `http` only (AD-1)"*.
...
- **Diff-to-plan reconciliation:** `git diff` every changed file against the plan's *Impacted
  Modules & Files*; flag files in the diff but not the plan (scope creep) **and** files in the plan
...
- **Review-completeness confidence** — scores *how thorough this review pass was*, not how good the
...
answering *"do we have to do all of this every time?"* — no, but then no gate caught anything.
...
scored. That single CSV *is* the compliance artifact.
...
Pick a small feature. One epic, two stories, ~3 files. The point is the *governance chain*, not the app.
